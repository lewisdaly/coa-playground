...
var tigerbeetle_default = "./tigerbeetle-q12w1t1m.wasm";
...
// src/import.ts
function getCell(row, index, offset = 0) {
  const val = row[index + offset];
  if (val === null || val === undefined)
    return "";
  return String(val).trim();
}
function getCellNum(row, index, offset = 0) {
  const val = row[index + offset];
  if (typeof val === "number")
    return val;
  return 0;
}
function detectOffset(rows) {
  for (let i = 0;i < Math.min(10, rows.length); i++) {
    ...
    if (col0 === "Schema" || col0 === "ID" || col0.startsWith("Case ") || col0.startsWith("gid=")) {
      return 0;
    }
    if (col1 === "Schema" || col1 === "ID" || col1.startsWith("Case ") || col1.startsWith("gid=")) {
      return 1;
    }
  }
  return 0;
}
function parseSchema(rows, startRow, offset) {
  const accounts = [];
  let i = startRow;
  if (rows[i] && getCell(rows[i], 0, offset) === "Schema") {
    i++;
  }
  if (rows[i] && getCell(rows[i], 0, offset) === "ID") {
    i++;
  }
  while (i < rows.length) {
    const row = rows[i];
    ...
    const firstCell = getCell(row, 0, offset);
    if (firstCell.startsWith("Case ")) {
      break;
    }
    if (!firstCell) {
      i++;
    ...
    const account = {
      id: firstCell,
      ledger: getCell(row, 2, offset),
      type: getCell(row, 3, offset),
      cardinality: getCell(row, 4, offset),
      flags: getCell(row, 5, offset),
      description: getCell(row, 6, offset)
    };
    if (account.id && account.ledger) {
      accounts.push(account);
    }
    i++;
  }
  return { accounts, endRow: i };
}
function parseCase(rows, startRow, offset) {
  let i = startRow;
  const row = rows[i];
  const caseId = getCell(row, 0, offset).replace("Case ", "");
  i++;
  const descRow = rows[i];
  const description = descRow ? getCell(descRow, 0, offset) : "";
  i++;
  while (i < rows.length) {
    const r = rows[i];
    if (r && getCell(r, 0, offset).includes("Example transfers")) {
      i++;
      break;
    }
    i++;
  }
  if (rows[i] && getCell(rows[i], 0, offset) === "#") {
    i++;
  }
  const transfers = [];
  let currentBatch = 0;
  while (i < rows.length) {
    ...
    const firstCell = getCell(r, 0, offset);
    if (firstCell.startsWith("Case ")) {
      break;
    }
    const ledger = getCell(r, 2, offset);
    if (!ledger) {
      i++;
      continue;
    }
    const batchVal = getCellNum(r, 0, offset);
    if (batchVal > 0) {
      currentBatch = batchVal;
    }
    const transfer = {
      batch: currentBatch,
      event: getCell(r, 1, offset),
      ledger,
      amount: getCellNum(r, 3, offset),
      debitAccount: getCell(r, 4, offset),
      debitType: getCell(r, 5, offset),
      creditAccount: getCell(r, 6, offset),
      creditType: getCell(r, 7, offset),
      flags: getCell(r, 8, offset),
      more: getCell(r, 9, offset),
      comments: getCell(r, 10, offset)
    };
    if (transfer.debitAccount && transfer.creditAccount) {
      transfers.push(transfer);
    }
    i++;
  }
  return {
    caseData: { id: caseId, description, transfers },
    endRow: i
  };
}
function parseSheet(sheet, sheetName) {
  ...
  while (i < rows.length && i < 10) {
    ...
    const cell = getCell(row, 0, offset);
    if (cell.startsWith("gid=")) {
      result.gid = cell.replace("gid=", "");
    } else if (cell === "Description:") {
      const descRow = rows[i + 1];
      result.description = descRow ? getCell(descRow, 0, offset) : "";
    } else if (cell === "Schema") {
      const { accounts, endRow } = parseSchema(rows, i, offset);
      result.schema = accounts;
      i = endRow;
      continue;
    } else if (cell.startsWith("Case ")) {
      const { caseData, endRow } = parseCase(rows, i, offset);
...
function parseWorkbookFromXLSX(workbook) {
  const sheets = [];
  for (const sheetName of workbook.SheetNames) {
    const sheet = workbook.Sheets[sheetName];
    if (sheet) {
      sheets.push(parseSheet(sheet, sheetName));
    }
  }
  return { sheets };
}

// src/playground-codegen.ts
function buildLedgerMapping(sheet) {
  const ledgers = new Map;
  let nextId = 1;
  for (const account of sheet.schema) {
    if (!ledgers.has(account.ledger))
      ledgers.set(account.ledger, nextId++);
  }
  return ledgers;
}
function buildAccountMapping(caseData, _schema, caseIndex) {
  const accounts = new Map;
  const baseId = BigInt(1e4 + caseIndex * 1000);
  let nextOffset = 0n;
  const usedAccounts = new Set;
  for (const t of caseData.transfers) {
    usedAccounts.add(t.debitAccount);
    usedAccounts.add(t.creditAccount);
  }
  for (const id of usedAccounts)
    accounts.set(id, baseId + nextOffset++);
  return accounts;
}
function flagsToString(flags) {
  const list = flags.toLowerCase().split(",").map((f) => f.trim()).filter((f) => f && f !== "none");
  return list.join(" | ");
}
function parseSchemaFlags(flags) {
  const list = [];
  const lower = flags.toLowerCase();
  if (lower.includes("cr≤dr") || lower.includes("cr<=dr"))
    list.push("credits_must_not_exceed_debits");
  if (lower.includes("dr≤cr") || lower.includes("dr<=cr"))
    list.push("debits_must_not_exceed_credits");
  if (lower.includes("credits_must_not_exceed_debits") && !list.includes("credits_must_not_exceed_debits"))
    list.push("credits_must_not_exceed_debits");
  if (lower.includes("debits_must_not_exceed_credits") && !list.includes("debits_must_not_exceed_credits"))
    list.push("debits_must_not_exceed_credits");
...
function parseMoreFieldRaw(more) {
  if (!more?.trim())
    return {};
  const result = {};
  for (const part of more.split(",").map((p) => p.trim()).filter(Boolean)) {
...
function formatMoreObject(parsed, batchPositionToId) {
  const fields = [];
  for (const [key, value] of Object.entries(parsed)) {
    if (key === "ref")
      continue;
    if (key === "fails_with") {
      fields.push(`failsWith: "${value}"`);
      continue;
    }
    if (key === "pending_id") {
      const resolved = batchPositionToId.get(value);
      if (resolved !== undefined)
        fields.push(`pending_id: ${resolved}n`);
      else if (/^\d+$/.test(value))
        fields.push(`pending_id: ${value}n`);
    } else if (["user_data_128", "user_data_64"].includes(key)) {
      fields.push(`${key}: ${value}n`);
    } else {
...
function generateCaseCode(caseData, schema, ledgerMapping, caseIndex) {
  const accountMapping = buildAccountMapping(caseData, schema, caseIndex);
  const lines = [];
  const funcName = `case${caseData.id.replace(/-/g, "_")}`;
  const descLines = caseData.description.split(`
`).filter((l) => l.trim());
  lines.push(`  // Case ${caseData.id}: ${descLines[0] ?? ""}`);
  for (let i = 1;i < descLines.length; i++)
    lines.push(`  // ${descLines[i]}`);
  lines.push(`  async function ${funcName}() {`);
  lines.push(`    log.section("Case ${caseData.id}", "${(descLines[0] ?? "").replace(/"/g, "\\\"")}");`);
  lines.push(`    resetAccountRegistry(${BigInt(1e4 + caseIndex * 1000)}n);`);
  lines.push("");
  const accountData = [];
  for (const [name] of accountMapping) {
    const sa = schema.find((a) => a.id === name);
    accountData.push({
      name,
      ledger: sa?.ledger ?? "CASH",
      ledgerConst: `LEDGER_${sa?.ledger ?? "CASH"}`,
      flags: sa?.flags ? parseSchemaFlags(sa.flags) : "",
      type: sa?.type ?? "Asset"
    });
  }
  const maxName = Math.max(...accountData.map((a) => a.name.length));
  const maxLedger = Math.max(...accountData.map((a) => a.ledgerConst.length));
  lines.push(`    const ctx = new CaseContext(tb, [`);
  for (const a of accountData) {
    lines.push(`      { id: "${a.name}", ledger: "${a.ledger}", type: "${a.type}", flags: "${a.flags}" },`);
  }
  lines.push(`    ]);`);
  lines.push("");
  lines.push("    await tb.createAccounts([");
  for (const a of accountData) {
    const n = `"${a.name}",`.padEnd(maxName + 3);
    const l = `${a.ledgerConst},`.padEnd(maxLedger + 1);
    lines.push(`      account({ name: ${n} ledger: ${l} flags: "${a.flags}" }),`);
  }
  lines.push("    ]);");
  lines.push(`    ctx.setAccountIds(getAccountId);`);
  lines.push("");
  ...
  const idBase = BigInt(1e4 + caseIndex * 1000);
  let idOffset = 0n;
  const posToId = new Map;
  const allBatches = [];
  for (const [batch, transfers] of byBatch) {
    const eventDesc = transfers[0]?.event ?? "";
    const batchTransfers = [];
    let pos = 1;
    for (const t of transfers) {
      const tid = idBase + idOffset++;
      const more = parseMoreFieldRaw(t.more ?? "");
      posToId.set(`${batch}.${pos++}`, tid);
      if (more.ref)
        posToId.set(more.ref, tid);
      batchTransfers.push({
        transferId: tid,
        ledgerConst: `LEDGER_${t.ledger}`,
        amount: t.amount,
        dr: t.debitAccount,
        cr: t.creditAccount,
        flags: flagsToString(t.flags),
        moreParsed: more
      });
    }
    allBatches.push({ batch, eventDesc, transfers: batchTransfers });
  }
  for (const { batch, eventDesc, transfers: bts } of allBatches) {
    const eventLines = eventDesc.split(`
`).filter((l) => l.trim()).map((l) => `"${l.replace(/"/g, "\\\"")}"`);
    const descArg = eventLines.length > 0 ? `[${eventLines.join(", ")}]` : `""`;
    const maxId = Math.max(...bts.map((t) => t.transferId.toString().length + 1));
    const maxL = Math.max(...bts.map((t) => t.ledgerConst.length));
    const maxAmt = Math.max(...bts.map((t) => t.amount.toString().length + 1));
    const maxDr = Math.max(...bts.map((t) => t.dr.length));
    const maxCr = Math.max(...bts.map((t) => t.cr.length));
    lines.push(`    await ctx.batch(${batch}, ${descArg}, [`);
    for (const t of bts) {
      const id = `${t.transferId}n,`.padEnd(maxId + 2);
      const lc = `${t.ledgerConst},`.padEnd(maxL + 1);
      const amt = `${t.amount}n,`.padStart(maxAmt + 2);
      const dr = `"${t.dr}",`.padEnd(maxDr + 4);
      const cr = `"${t.cr}",`.padEnd(maxCr + 4);
      const more = formatMoreObject(t.moreParsed, posToId);
      if (t.flags && more)
        lines.push(`      transfer(${id} ${lc} ${amt} ${dr} ${cr} "${t.flags}", ${more}),`);
      else if (t.flags)
        lines.push(`      transfer(${id} ${lc} ${amt} ${dr} ${cr} "${t.flags}"),`);
      else if (more)
        lines.push(`      transfer(${id} ${lc} ${amt} ${dr} ${cr} "", ${more}),`);
      else
        lines.push(`      transfer(${id} ${lc} ${amt} ${dr} ${cr.trimEnd().slice(0, -1)}),`);
    }
    lines.push(`    ]);`);
    lines.push("");
  }
  lines.push(`    log.dim("Case ${caseData.id} complete.");`);
  lines.push("  }");
  return lines.join(`
`);
}
function generateRecipe(sheet) {
  const ledgerMapping = buildLedgerMapping(sheet);
  const lines = [];
  lines.push(`// Generated from Sheet: ${sheet.name}`);
  if (sheet.description) {
    for (const line of sheet.description.split(`
`))
      lines.push(`// ${line}`);
  }
  lines.push("");
  lines.push('import type { RecipeContext } from "@/runner";');
  lines.push('import { account, transfer, resetAccountRegistry, getAccountId } from "@/tigerbeetle";');
  lines.push('import { log } from "@/log";');
  lines.push('import { CaseContext } from "@/t-accounts";');
  lines.push("");
  for (const [name, id] of ledgerMapping)
    lines.push(`const LEDGER_${name} = ${id};`);
  lines.push("");
  lines.push("function shouldRunCase(caseId: string, filter?: string): boolean {");
  lines.push("  if (!filter) return true;");
  lines.push("  return caseId.startsWith(filter) || caseId.includes(filter);");
  lines.push("}");
  lines.push("");
  lines.push("export default async function recipe(ctx: RecipeContext): Promise<void> {");
  lines.push("  const { tb, caseFilter } = ctx;");
  lines.push("");
  for (let i = 0;i < sheet.cases.length; i++) {
    lines.push(generateCaseCode(sheet.cases[i], sheet.schema, ledgerMapping, i));
    lines.push("");
  }
  for (const c of sheet.cases) {
    const fn = `case${c.id.replace(/-/g, "_")}`;
    lines.push(`  if (shouldRunCase("${c.id}", caseFilter)) await ${fn}();`);
  }
  lines.push("}");
  return lines.join(`
`);
...
class WasmTigerbeetle {
  ...
  constructor(instance) {
    ...
    this.handle = this.ex.tb_init();
    if (this.handle === 0)
      throw new Error("tb_init failed: out of memory");
  }
  static async load(wasmUrl) {
    const result = await WebAssembly.instantiateStreaming(fetch(wasmUrl), {});
    return new WasmTigerbeetle(result.instance);
  }
  reset() {
    this.ex.tb_deinit(this.handle);
    this.handle = this.ex.tb_init();
    if (this.handle === 0)
      throw new Error("tb_init failed: out of memory");
    this.accountIds = [];
  ...
  createAccounts(accounts) {
    ...
    if (n === 0)
      return [];
    const accsSize = n * ACCOUNT_SIZE;
    const errSize = n * ERROR_SIZE;
    const accsPtr = this.alloc(accsSize);
    const errPtr = this.alloc(errSize);
    try {
      let v = this.view();
      for (let i = 0;i < n; i++) {
        writeAccount(v, accsPtr + i * ACCOUNT_SIZE, accounts[i]);
      }
      const errCount = this.ex.tb_create_accounts(this.handle, accsPtr, n, errPtr);
      const errors = [];
      v = this.view();
      for (let i = 0;i < errCount; i++) {
        const base = errPtr + i * ERROR_SIZE;
        const index = v.getUint32(base, true);
        const code = v.getUint32(base + 4, true);
        errors.push({ index, result: CREATE_ACCOUNT_RESULTS[code] ?? `error_${code}` });
      }
      const failed = new Set(errors.map((e) => e.index));
      for (let i = 0;i < n; i++) {
        if (!failed.has(i))
          this.accountIds.push(accounts[i].id);
      }
      return errors;
    } finally {
      this.ex.tb_free(accsPtr, accsSize);
      this.ex.tb_free(errPtr, errSize);
    }
  }
  createTransfers(transfers) {
    ...
    if (n === 0)
      return [];
    const trsSize = n * TRANSFER_SIZE;
    const errSize = n * ERROR_SIZE;
    const trsPtr = this.alloc(trsSize);
    const errPtr = this.alloc(errSize);
    try {
      let v = this.view();
      for (let i = 0;i < n; i++) {
        writeTransfer(v, trsPtr + i * TRANSFER_SIZE, transfers[i]);
      }
      const errCount = this.ex.tb_create_transfers(this.handle, trsPtr, n, errPtr);
      const errors = [];
      v = this.view();
      for (let i = 0;i < errCount; i++) {
        const base = errPtr + i * ERROR_SIZE;
        const index = v.getUint32(base, true);
        const code = v.getUint32(base + 4, true);
        console.log(`[DEBUG] Transfer error: index=${index}, code=${code}, mapped=${CREATE_TRANSFER_RESULTS[code] ?? `error_${code}`}`);
        errors.push({ index, result: CREATE_TRANSFER_RESULTS[code] ?? `error_${code}` });
      }
      return errors;
    } finally {
      this.ex.tb_free(trsPtr, trsSize);
      this.ex.tb_free(errPtr, errSize);
    }
  }
  getAllAccounts() {
    const n = this.accountIds.length;
    if (n === 0)
      return [];
    const idsSize = n * 16;
    const outSize = n * ACCOUNT_SIZE;
    const idsPtr = this.alloc(idsSize);
    const outPtr = this.alloc(outSize);
    try {
      let v = this.view();
      for (let i = 0;i < n; i++) {
        writeU128(v, idsPtr + i * 16, this.accountIds[i]);
      }
      const count = this.ex.tb_lookup_accounts(this.handle, idsPtr, n, outPtr);
      const result = [];
      v = this.view();
      for (let i = 0;i < count; i++) {
        result.push(readAccount(v, outPtr + i * ACCOUNT_SIZE));
      }
      return result;
    } finally {
      this.ex.tb_free(idsPtr, idsSize);
      this.ex.tb_free(outPtr, outSize);
    }
  }
  lookupAccounts(ids) {
    const n = ids.length;
  ...
  lookupTransfers(ids) {
    ...
    try {
      ...
      const count = this.ex.tb_lookup_transfers(this.handle, idsPtr, n, outPtr);
      const result = [];
      v = this.view();
      for (let i = 0;i < count; i++) {
        result.push(readTransfer(v, outPtr + i * TRANSFER_SIZE));
      }
      return result;
    } finally {
      this.ex.tb_free(idsPtr, idsSize);
  ...
  getAccountTransfers(filter) {
    const outSize = filter.limit * TRANSFER_SIZE;
    const filterPtr = this.alloc(ACCOUNT_FILTER_SIZE);
    const outPtr = this.alloc(outSize);
    try {
      let v = this.view();
      writeAccountFilter(v, filterPtr, filter);
      const count = this.ex.tb_get_account_transfers(this.handle, filterPtr, outPtr);
      v = this.view();
      const result = [];
      for (let i = 0;i < count; i++)
        result.push(readTransfer(v, outPtr + i * TRANSFER_SIZE));
      ...
      this.ex.tb_free(filterPtr, ACCOUNT_FILTER_SIZE);
      this.ex.tb_free(outPtr, outSize);
    }
  }
  getAccountBalances(filter) {
    const outSize = filter.limit * ACCOUNT_BALANCE_SIZE;
    const filterPtr = this.alloc(ACCOUNT_FILTER_SIZE);
    const outPtr = this.alloc(outSize);
    try {
      let v = this.view();
      writeAccountFilter(v, filterPtr, filter);
      const count = this.ex.tb_get_account_balances(this.handle, filterPtr, outPtr);
      v = this.view();
      const result = [];
      for (let i = 0;i < count; i++)
        result.push(readAccountBalance(v, outPtr + i * ACCOUNT_BALANCE_SIZE));
      return result;
    } finally {
      this.ex.tb_free(filterPtr, ACCOUNT_FILTER_SIZE);
      this.ex.tb_free(outPtr, outSize);
  ...
  queryAccounts(filter) {
    const outSize = filter.limit * ACCOUNT_SIZE;
    const filterPtr = this.alloc(QUERY_FILTER_SIZE);
    const outPtr = this.alloc(outSize);
    try {
      let v = this.view();
      writeQueryFilter(v, filterPtr, filter);
      const count = this.ex.tb_query_accounts(this.handle, filterPtr, outPtr);
      v = this.view();
      const result = [];
      for (let i = 0;i < count; i++)
        result.push(readAccount(v, outPtr + i * ACCOUNT_SIZE));
      ...
      this.ex.tb_free(filterPtr, QUERY_FILTER_SIZE);
      this.ex.tb_free(outPtr, outSize);
    }
  }
  queryTransfers(filter) {
    ...
    try {
      let v = this.view();
      writeQueryFilter(v, filterPtr, filter);
      const count = this.ex.tb_query_transfers(this.handle, filterPtr, outPtr);
      v = this.view();
      const result = [];
      for (let i = 0;i < count; i++)
        result.push(readTransfer(v, outPtr + i * TRANSFER_SIZE));
...
var sheetUrlEl = document.getElementById("sheet-url");
...
async function loadFile(name) {
  ...
  if (hasUnsavedChanges && currentFileHandle) {
    ...
    if (!discard) {
      fileSelect.value = currentFileHandle.name;
      return;
    }
  }
  try {
...
function showFolderUI(show) {
  fileSelect.style.display = show ? "" : "none";
  saveBtn.style.display = show ? "" : "none";
  newFileBtn.style.display = show ? "" : "none";
}
function updateSaveButton() {
  saveBtn.textContent = hasUnsavedChanges ? "Save*" : "Save";
}
folderBtn.addEventListener("click", () => void openFolder());
fileSelect.addEventListener("change", () => void loadFile(fileSelect.value));
saveBtn.addEventListener("click", () => void saveCurrentFile());
newFileBtn.addEventListener("click", () => void createNewFile());
var tAccountsTabs = [];
...
function createTab(name) {
  ...
  panelEl.id = id;
  const tab = { id, name, tabEl, panelEl };
...
async function saveCodeToUrl(code) {
  const encoded = await compressToUrl(code);
  const newUrl = `${window.location.pathname}#code=${encoded}`;
  window.history.replaceState(null, "", newUrl);
}
var urlUpdateTimeout = null;
function debouncedSaveCodeToUrl(code) {
  if (urlUpdateTimeout)
    clearTimeout(urlUpdateTimeout);
  urlUpdateTimeout = setTimeout(() => void saveCodeToUrl(code), 500);
}
var DEFAULT_CODE = `// Define ledgers
const INSTR = defineLedger("INSTR", 1);
const CASH = defineLedger("CASH", 2);
const AUTH = defineLedger("AUTH", 3);

// ─── Case 001-001 ───────────────────────────────────────────────────────────
// Demonstrates how a cash authorization is created in the simple case.
async function case001_001() {
  resetAccountRegistry(10000n, "case001_001");
  const ctx = new CaseContext();

  await ctx.createAccounts([
    account({ name: "brk:etf1", ledger: INSTR, type: "Asset", flags: "credits_must_not_exceed_debits" }),
    account({ name: "c1:p1:etf1", ledger: INSTR, type: "Liability", flags: "debits_must_not_exceed_credits" }),
    account({ name: "c2:p1:etf1", ledger: INSTR, type: "Liability", flags: "debits_must_not_exceed_credits" }),
    account({ name: "brk:cash:cleared", ledger: CASH, type: "Asset", flags: "" }),
    account({ name: "c1:p1:cash", ledger: CASH, type: "Liability", flags: "debits_must_not_exceed_credits" }),
    account({ name: "c2:p1:cash", ledger: CASH, type: "Liability", flags: "debits_must_not_exceed_credits" }),
    account({ name: "auth:cash", ledger: AUTH, type: "Asset", flags: "credits_must_not_exceed_debits" }),
    account({ name: "auth:ctl", ledger: AUTH, type: "Liability", flags: "" }),
    account({ name: "brk:cash", ledger: CASH, type: "Asset", flags: "credits_must_not_exceed_debits" }),
  ]);

  // Batch 0: Setup opening balances
  await ctx.batch(0, [
    "Setup opening balances",
    "c1 has 50 etf1 in portfolio, c2 has 50 etf1 in portfolio",
  ], [
    transfer(10000n, INSTR, 50n, "brk:etf1", "c1:p1:etf1"),
    transfer(10001n, INSTR, 50n, "brk:etf1", "c2:p1:etf1"),
  ]);

  // Batch 1: t=0 - Order sent to market
  await ctx.batch(1, [
    "t=0 - Order sent to market. Sell 100x etf1 for 1000",
    "Reserve the estimated quantity of EFT1 across 2 portfolios.",
  ], [
    transfer(10002n, INSTR, 50n, "c1:p1:etf1", "brk:etf1", "pending"),
    transfer(10003n, CASH, 500n, "brk:cash:cleared", "c1:p1:cash", "pending"),
    transfer(10004n, INSTR, 50n, "c2:p1:etf1", "brk:etf1", "pending"),
    transfer(10005n, CASH, 500n, "brk:cash:cleared", "c2:p1:cash", "pending"),
  ]);

  // Batch 2: t=0.1 - Fill message received
  await ctx.batch(2, "t=0.1 - Fill message received.", [
    transfer(10006n, INSTR, 50n, "c1:p1:etf1", "brk:etf1", "post_pending_transfer", { pending_id: 10002n }),
    transfer(10007n, CASH, 500n, "brk:cash:cleared", "c1:p1:cash", "post_pending_transfer", { pending_id: 10003n }),
    transfer(10008n, INSTR, 50n, "c2:p1:etf1", "brk:etf1", "post_pending_transfer", { pending_id: 10004n }),
    transfer(10009n, CASH, 500n, "brk:cash:cleared", "c2:p1:cash", "post_pending_transfer", { pending_id: 10005n }),
    transfer(10010n, AUTH, 1000n, "auth:cash", "auth:ctl"),
    transfer(10011n, AUTH, 1000n, "auth:ctl", "auth:cash", "pending"),
  ]);

  // Batch 3: t=2 - Settlement occurs
  await ctx.batch(3, "t=2 - Settlement occurs.", [
    transfer(10012n, CASH, 1000n, "brk:cash", "brk:cash:cleared"),
    transfer(10013n, AUTH, 1000n, "auth:ctl", "auth:cash", "post_pending_transfer", { pending_id: 10011n }),
  ]);
}

// ─── Case 001-002 ───────────────────────────────────────────────────────────
// Demonstrates how a cash authorization can be used by a BUY.
async function case001_002() {
  resetAccountRegistry(11000n, "case001_002");
  const ctx = new CaseContext();
  ...
  await ctx.createAccounts([
    ...
    account({ name: "auth:cash", ledger: AUTH, type: "Asset", flags: "credits_must_not_exceed_debits" }),
    account({ name: "auth:ctl", ledger: AUTH, type: "Liability", flags: "" }),
    account({ name: "brk:etf2:cleared", ledger: INSTR, type: "Asset", flags: "credits_must_not_exceed_debits" }),
    account({ name: "c1:p1:etf2", ledger: INSTR, type: "Liability", flags: "debits_must_not_exceed_credits" }),
    account({ name: "brk:etf2", ledger: INSTR, type: "Asset", flags: "credits_must_not_exceed_debits" }),
  ]);

  // Batch 0: Setup opening balances
  await ctx.batch(0, "Setup opening balances - c1 has 10 etf1 in portfolio", [
    transfer(11000n, INSTR, 10n, "brk:etf1", "c1:p1:etf1"),
    transfer(11001n, CASH, 1000n, "brk:cash", "c1:p1:cash"),
  ]);

  // Batch 1: t=0.1 - SELL fill received
  await ctx.batch(1, "t=0.1 - SELL 10x etf1 for 1000. Fill message received.", [
    transfer(11002n, INSTR, 10n, "c1:p1:etf1", "brk:etf1"),
    transfer(11003n, CASH, 1000n, "brk:cash:cleared", "c1:p1:cash"),
    transfer(11004n, AUTH, 1000n, "auth:cash", "auth:ctl"),
    transfer(11005n, AUTH, 1000n, "auth:ctl", "auth:cash", "pending"),
  ]);

  // Batch 2: t=0.2 - BUY fill received
  await ctx.batch(2, "t=0.2 - BUY 10x etf2 for 900. Fill message received.", [
    transfer(11006n, INSTR, 10n, "brk:etf2:cleared", "c1:p1:etf2"),
    transfer(11007n, CASH, 900n, "c1:p1:cash", "brk:cash:cleared"),
    transfer(11008n, AUTH, 1000n, "auth:ctl", "auth:cash", "void_pending_transfer", { pending_id: 11005n }),
    transfer(11009n, AUTH, 900n, "auth:ctl", "auth:cash"),
    transfer(11010n, AUTH, 100n, "auth:ctl", "auth:cash", "pending"),
  ]);

  // Batch 3: t=0.3 - Try to authorize a BUY that exceeds available auth
  await ctx.batch(3, [
    "t=0.3 - Try to authorize a BUY of 2x etf2 for 180.",
    "Rejected as only 100 is available.",
  ], [
    transfer(11011n, INSTR, 2n, "brk:etf2:cleared", "c1:p1:etf2"),
    transfer(11012n, CASH, 180n, "c1:p1:cash", "brk:cash:cleared"),
    transfer(11013n, AUTH, 100n, "auth:ctl", "auth:cash", "void_pending_transfer", { pending_id: 11010n }),
    transfer(11014n, AUTH, 180n, "auth:ctl", "auth:cash", "", { failsWith: "exceeds_debits" }),
  ]);

  // Batch 4: t=2 - Settlement date arrives. Settle SELL.
  await ctx.batch(4, "t=2 - Settlement date arrives. Settle SELL.", [
    transfer(11015n, CASH, 1000n, "brk:cash", "brk:cash:cleared"),
    transfer(11016n, AUTH, 100n, "auth:ctl", "auth:cash", "post_pending_transfer", { pending_id: 11010n }),
  ]);

  // Batch 5: t=2.1 - Settlement date arrives. Settle BUY.
  await ctx.batch(5, "t=2.1 - Settlement date arrives. Settle BUY.", [
    transfer(11017n, CASH, 900n, "brk:cash:cleared", "brk:cash"),
    transfer(11018n, INSTR, 10n, "brk:etf2", "brk:etf2:cleared"),
  ]);
}

// ─── Run cases ──────────────────────────────────────────────────────────────
// Comment/uncomment to run specific cases:
await case001_001();
// await case001_002();
`;
var initialCode = await getCodeFromUrl() ?? DEFAULT_CODE;
...
monacoEditor.onDidChangeModelContent(() => {
  debouncedSaveCodeToUrl(monacoEditor.getValue());
...
async function saveAndCopyUrl() {
  await saveCodeToUrl(monacoEditor.getValue());
  try {
    await navigator.clipboard.writeText(window.location.href);
    setStatus("URL copied to clipboard", "ok");
  } catch {
    setStatus("Saved to URL", "ok");
  }
}
...
function toggleConsoleMaximize() {
  ...
  editorContainerEl.style.display = isMaximized ? "none" : "";
...
var tb = await WasmTigerbeetle.load(tigerbeetle_default);
setStatus("Ready", "ok");
...
if (loadBtn && sheetUrlEl) {
  loadBtn.addEventListener("click", () => void loadSheet());
}
async function loadSheet() {
  if (!sheetUrlEl || !loadBtn)
    return;
  ...
  try {
    ...
    const parsed = parseWorkbookFromXLSX(wb);
    const sheet = parsed.sheets.find((s) => s.cases.length > 0);
    if (!sheet)
      throw new Error("No cases found in this sheet.");
    const code = generateRecipe(sheet);
...
function fmtAmtPad(n, width = 12) {
  return fmtAmt(n).padStart(width);
}
...
function renderTAccountCard(acc, name, allTransfers, voidedIds) {
  const accountType = getAccountType(acc);
  ...
  for (const t of debits) {
    ...
    debitRows += `<div class="t-entry debit-entry${voidedClass}" data-tid="${esc(t.id.toString())}">
      <span class="entry-amount">${esc(fmtAmt(t.amount))}</span><span class="entry-pending">${isPending ? "(p)" : ""}</span>
    </div>`;
  }
  let creditRows = "";
  for (const t of credits) {
    ...
    const isVoided = voidedIds.has(t.id.toString());
    const voidedClass = isVoided ? " voided" : "";
    creditRows += `<div class="t-entry credit-entry${voidedClass}" data-tid="${esc(t.id.toString())}">
      <span class="entry-amount">${esc(fmtAmt(t.amount))}</span><span class="entry-pending">${isPending ? "(p)" : ""}</span>
    </div>`;
  }
  ...
  let netCrDisplay = "";
  if (drTotal > crTotal) {
    netDrDisplay = fmtAmt(drTotal - crTotal);
  } else if (crTotal > drTotal) {
    netCrDisplay = fmtAmt(crTotal - drTotal);
  } else {
    if (isDebitNormal) {
      netDrDisplay = "0";
    } else {
  ...
  card.innerHTML = `
    <div class="t-account-header">
      <div class="t-account-header-left">
        <span class="t-account-type">${esc(accountType)}</span>
        <span class="t-account-name">${esc(name)}</span>
      </div>
      <span class="t-account-ledger ${ledgerColorClass}">${esc(ledgerName)}</span>
    </div>
    ...
    <div class="t-totals-row">
      <div class="t-total t-total-debit">
        <div class="t-total-inner">
          <span class="t-total-amount">${esc(netDrDisplay)}</span><span class="t-total-spacer"></span>
        </div>
      </div>
      <div class="t-total-divider"></div>
      <div class="t-total t-total-credit">
        <div class="t-total-inner">
          <span class="t-total-amount">${esc(netCrDisplay)}</span><span class="t-total-spacer"></span>
        </div>
      </div>
    </div>
  `;
...
function renderTAccounts(allTransfers) {
  ...
  function renderTypeSection(title, className, ledgerMap) {
    ...
    for (const [ledgerId, accs] of ledgerMap) {
      ...
      ledgerRow.className = "t-accounts-ledger-row";
      const ledgerLabel = document.createElement("div");
      ledgerLabel.className = `t-accounts-ledger-label ledger-color-${ledgerId % 6}`;
      ledgerLabel.textContent = ledgerName;
      ledgerRow.appendChild(ledgerLabel);
      const cardsRow = document.createElement("div");
      cardsRow.className = "t-accounts-row";
      for (const acc of accs) {
        const name = names.get(acc.id) ?? acc.id.toString();
        cardsRow.appendChild(renderTAccountCard(acc, name, allTransfers, voidedIds));
      }
      ledgerRow.appendChild(cardsRow);
      section.appendChild(ledgerRow);
    }
